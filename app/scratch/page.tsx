...
export default function ScratchPage() {
  ...
  const remove = async (id: number) => {
    if (!confirm('確定要刪除這個刮刮樂種類嗎？')) return
    await fetch(`/api/scratch/${id}`, { method: 'DELETE' })
    fetch_()
  }

  // Group by price
  const grouped = tickets.reduce<{ [price: number]: ScratchTicket[] }>((acc, t) => {
            ...
            .map(([price, items]) => (
              <div key={price}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-amber-500">{items.length} 種</span>
                </div>
                <div className="bg-white border border-amber-200 rounded-xl overflow-hidden shadow-sm">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-amber-50 border-b border-amber-100">
                        <th className="text-left px-4 py-2.5 text-xs font-semibold text-amber-700">名稱</th>
                        <th className="text-center px-4 py-2.5 text-xs font-semibold text-amber-700">每本張數</th>
                        <th className="text-left px-4 py-2.5 text-xs font-semibold text-amber-700">備註</th>
                    ...
                    <tbody className="divide-y divide-amber-50">
                      {items.map((t) => (
                        <tr key={t.id} className="hover:bg-amber-50 transition-colors">
                          <td className="px-4 py-3 font-medium text-amber-950">{t.name}</td>
                          <td className="px-4 py-3 text-center text-amber-800">{t.sheetsPerBook} 張</td>
                          <td className="px-4 py-3 text-amber-500 text-xs">{t.note ?? '—'}</td>
                          <td className="px-4 py-3 text-right whitespace-nowrap">
                            <button onClick={() => openEdit(t)} className="text-xs text-amber-700 hover:underline font-medium mr-3">編輯</button>
                            <button onClick={() => remove(t.id)} className="text-xs text-red-400 hover:underline">刪除</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
          <div className="bg-white rounded-xl shadow-xl p-6 w-96 border-t-4 border-amber-400">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-amber-900 mb-1">名稱</label>
                <input
              ...
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="block text-sm font-medium text-amber-900 mb-1">面額（元）</label>
                  <input
                    type="number"
                    min="0"
                    className="w-full border border-amber-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
                ...
                <div className="flex-1">
                  <label className="block text-sm font-medium text-amber-900 mb-1">每本張數</label>
                  <input
                    type="number"
                    min="1"
                    className="w-full border border-amber-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
