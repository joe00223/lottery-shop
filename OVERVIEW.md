...
- 記錄各票種未拆封書、已拆書、陳列張數、補張數（restockSheets）
- 補書按鈕（點「補張」切換鈕後顯示）：補入未拆封書，同步在庫存管理新增 OUT 紀錄，並累加 restockSheets
- 未開封 = 0 且庫存管理淨庫存也 = 0 時，格子顯示紅色 ✕（值仍為 0，不影響計算）
- 補張欄與補書按鈕預設隱藏，工具列「補張」切換鈕開啟後才顯示
- 拖曳排序票種顯示順序
...
- 游標在畫面底部時，選員工 picker 自動往上彈，避免選項超出螢幕
